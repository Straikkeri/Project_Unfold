...
public class Creature {
    ...
    private int hasHerd;
    private int isBurrower;
    private int hasFangs;
    private int hasClaws;
    private int canProwl;
    private int hasTusks;
    ...
    private boolean herbivoreHasFedWell = false;
    ...
    public Creature runCreature(Square[][] squares,ArrayList<Creature> creatures, int columns, int rows, int iterator){
        ...
        double anxIndex = determineAnxiousness(foodWhereIStand, shelterWhereIStand, temperatureWhereIStand, terrainWhereIStand, iterator);
        
        if (!doneOnce)
        {
            this.creatures.get(iterator).diet  = (int) (Math.random() * 2);
            this.creatures.get(iterator).isFlying = (int) (Math.random() * 2+0.5);
            this.creatures.get(iterator).hasHerd = (int) (Math.random() * 2+0.5);
            this.creatures.get(iterator).isBurrower = (int) (Math.random() * 2+0.5);
            this.creatures.get(iterator).hasFangs = (int) (Math.random() * 2+0.5);
            this.creatures.get(iterator).hasClaws = (int) (Math.random() * 2+0.5);
            this.creatures.get(iterator).canProwl = (int) (Math.random() * 2+0.5);
            this.creatures.get(iterator).hasTusks = (int) (Math.random() * 2+0.5);
        ...
        {
            // jos kasvissyöjä
            if (this.creatures.get(iterator).getDiet() == 0)
            {
                // jos kasvissyöjällä on energiavarastot vajaat ja ruudussa on edes vähän ruokaa, se syö
                if(energyReserve < 6 && this.squares[x][y].getFood() > 0)
                {
                    this.creatures = feed(this.squares,this.creatures, iterator);
                   // returnCreature = feed(this.squares,this.creatures, iterator);
                }
                // jos kasvissyöjällä on energiavarastot riittävät, se liikkuu paremmille maille
                if(energyReserve >= 6)
                {
            ...
            // jos lihansyöjä
            if (this.creatures.get(iterator).getDiet() == 1)
            {
                // jos lihansyöjällä vajaat energiavarastot, se etsii syötävää
                if(energyReserve >= 15)
                {
                    move(this.squares, columns, rows);
                }
                // KORJAA JOS TARVITSEE, TÄYTYY EHKÄ TEHDÄ ERILLINEN METODI NÄLKIINTYMISELLE
                
                if(energyReserve < 15)
                {
                    for(int i = 0;i<squares[x][y].inhabitants.size();i++)
                    {
                        if(squares[x][y].inhabitants.contains(squares[x][y].inhabitants.get(i).size <= this.size && squares[x][y].inhabitants.get(i).id != this.id) ||  squares[x][y].inhabitants.get(i).hasHerd == 0 && this.hasHerd == 1 &&  squares[x][y].inhabitants.get(i).id != this.id)
                        {
                            this.creatures = feed(this.squares,this.creatures, iterator);
                            i = squares[x][y].inhabitants.size();
                        }
                    }                                       
                }
                if (energyReserve < 15)
       ...
       /* for(int i = 0;i<creatures.size();i++)
        {
            System.out.println("Olen "+id+" ja diet runcreaturen lopussa on "+this.creatures.get(i).getDiet());
        }*/
        return this;
    }
    ...
    private double determineAnxiousness(double food, double shelter, double temperature, int terrain, int iterator){

        double anxIndex = 0.0;
       
        anxIndex = determineHungerAnx(food, anxIndex, iterator);
   ...
   /* private boolean Hunger(ArrayList<Creature>creatures, Square[][]squares, int x, int y, int iterator)
    {
        if(diet == 0)
        {            
            if (creatures.get(iterator).herbivoreHasFedWell)
            {
                creatures.get(iterator).energyReserve++;
            }
        }
    ...
    private void carnivoreFeed()
    {
        
    }
    
    private void herbivoreFeed()
    {
        
    }
    
    private void omnivoreFeed()
    {
        
    }
    
    private ArrayList feed(Square[][]squares,ArrayList<Creature> creatures, int iterator)
    {
       ...
       // hasFed = false;
        //herbivoreHasFedWell = false;

            System.out.println("I am: " + creatures.get(iterator).id + ".Feed diet: " + creatures.get(iterator).getDiet());
        ...
        if (this.creatures.get(iterator).getDiet() == 1)
        {
            ...
            if (this.energyReserve<20)
            {
                System.out.println("Minulla on nälkä");
               //System.out.println(this.inhabitants.size());
                ...
                for(int i = 0;i<this.inhabitants.size();i++)
                {
                    ...
                    if (this.inhabitants.get(i).size <= this.size && this.inhabitants.get(i).id != this.id || this.inhabitants.get(i).hasHerd == 0 && this.hasHerd == 1 && this.inhabitants.get(i).id != this.id)
                    {
                        ...
                        // kiinni jäi
                        if(hunt(this.inhabitants.get(i).getSurvivalFeatures(), this.getSurvivalFeatures()))
                        {
                            System.out.println("Saalis saatiin kiinni, lisätään energiaa varastoon "+energyReserve+"(energyReserve) +"+this.inhabitants.get(i).carcassEnergyValue+"carcassEnergyValue");
                            ...
                            this.creatures.remove(this.inhabitants.get(i));
                            i = this.inhabitants.size();
                        }
                        //pääsi karkuun
                        else
                        {
    ...
    private double determineHungerAnx(double food,double anxIndex, int iterator)
    {
       // System.out.println("food in square " + y + "." + x + ": " + food);


        // jos kasvissyöjä tai kaikkiruokainen
        if(creatures.get(iterator).getDiet() == 0 || creatures.get(iterator).getDiet() == 2)
        {
        ...
        // jos lihansyöjä tai kaikkiruokainen
        if(creatures.get(iterator).getDiet() == 1 || creatures.get(iterator).getDiet() == 2)
        {
